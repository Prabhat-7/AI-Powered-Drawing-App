...
export default function Canvas() {
  ...
  function startDrawing(e: MouseEvent<HTMLCanvasElement>) {
    ...
    if (!contx) return;
    contx.strokeStyle = isEraser ? "#1e1e1e" : color;
  ...
  async function sendImageToBackend(imageData: string): Promise<void> {
    try {
      ...
      if (response.ok) setSolution(data.response);
      else throw new Error(data.error || "failed to process the image");
    } catch (err) {
      console.log(err);
      setSolution("Error processing the data.");
    }
  }
  const handleSolveEquation = (): void => {
  ...
  return (
    <div className="canvas">
      <header className="canvas-header">Draw and Solve</header>
